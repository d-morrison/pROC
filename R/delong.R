...
delong.unpaired.test <- function(calcs) {
  ...
  if (is.nan(t) && calcs$d == 0 && calcs$sig == 0) {
    t <- 0
  } # special case: no difference between theta's produces a NaN
...
# function to calculate the CI for paired or unpaired test
ci_delong <- function(calcs, conf.level) {
  # Input calcs is a list generated by delong.paired.calculations() or delong.unpaired.calculations().
  # For paired tests: CI is calculated using the normally distributed pivot given in DeLong's 1988 paper.
  # For unpaired tests: CI is calculated using a t-distribution with Welch-Satterthwaite degrees of freedom,
  #   which provides more appropriate inference for independent samples.

  # Check if this is an unpaired test (has degrees of freedom) or paired test
  if (!is.null(calcs$df)) {
    # Unpaired: use t-distribution with calculated degrees of freedom
    crit_val <- qt(1 - ((1 - conf.level) / 2), df = calcs$df)
  } else {
    # Paired: use normal distribution
    crit_val <- qnorm(1 - ((1 - conf.level) / 2))
  }
  
  out <- list()
  out$upper <- with(calcs, d + crit_val * sig)
  out$lower <- with(calcs, d - crit_val * sig)
  out$level <- conf.level
  return(out)
}

# Backwards compatibility aliases
ci_delong_paired <- ci_delong
ci_delong_unpaired <- ci_delong
